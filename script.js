...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    const speed = 400; // Speed in milliseconds
    let index = 0;
    ...
    function typeWriter() {
        ...
        if (isDeleting) {
            ...
            if (index < 0) {
                isDeleting = false;
                setTimeout(typeWriter, 10000); // Delay before starting to type again
            } else {
            ...
            if (index > text.length) {
                isDeleting = true;
                setTimeout(typeWriter, 20000); // Delay before starting to delete
            } else {
